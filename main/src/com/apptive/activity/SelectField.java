...
public class SelectField extends BaseActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
        ...
        int count;
	    Cursor mCount = MainActivity.mdb.rawQuery("SELECT count(*) from fieldcategory", null);
		mCount.moveToFirst();
		count = mCount.getInt(0);
		mCount.close();
        Cursor result = MainActivity.mdb.rawQuery("SELECT * from fieldcategory", null);
        result.moveToFirst();
    
        int childCount;
        mGroupList.add("모든 분야");
		...
		expandablelist.setOnChildClickListener(new OnChildClickListener() {
			...
			public boolean onChildClick(ExpandableListView parent, View v,int groupPosition, int childPosition, long id) {
				...
				DetailSearch.field = mChildList.get(groupPosition).get(childPosition).toString();
				//리스트 갱신
		...
		expandablelist.setOnGroupExpandListener(new OnGroupExpandListener() {
			...
			public void onGroupExpand(int groupPosition) {
				...
				if(groupPosition == 0){
					DetailSearch.field = mGroupList.get(groupPosition).toString();
					DetailSearch.mSelectListAdapter.notifyDataSetChanged();
					finish();
				}
